...
export function simulate(p){const g=structure(p),nodes=new Map(p.nodes.map(n=>[n.id,n])),visits=new Map(),trace=[],limit=Math.min(p.settings.maxSteps||10000,10000);let pending=p.nodes.filter(n=>n.type==='start').map(n=>({id:n.id,round:1})),rounds=0,stopped=false;
 ...
 else if(n.type==='guardrail'&&outgoing.length>1){outgoing=n.route?outgoing.filter(e=>e.label===n.route).slice(0,1):[];if(!outgoing.length)stopped=true;}
 else if(['router','human'].includes(n.type)&&outgoing.length>1){outgoing=n.route?outgoing.filter(e=>e.label===n.route).slice(0,1):outgoing.slice(0,1);if(!outgoing.length)stopped=true;}
